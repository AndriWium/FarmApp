...
public class SeasonService(ISeasonRepository repo, IPlantingRepository plantingRepo, IUnitOfWork uow) : ISeasonService
{
    ...
    private static Expression<Func<Season, SeasonDto>> ToDtoExpr()
        => s => new SeasonDto(s.SeasonId, s.PlantingId, s.Name, s.StartDate, s.EndDate);

    private static SeasonDto ToDto(Season s) => new(s.SeasonId, s.PlantingId, s.Name, s.StartDate, s.EndDate);
}
