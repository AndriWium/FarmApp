...
public record ProducePurchaseLineDto(
    int ProducePurchaseLineId, int ProductId, int? GradeId, decimal Qty, decimal UnitCost, decimal? VatAmount, int StockBatchId);
...
/// to build the line's StockBatch, which requires it (see DECISIONS.md). VatAmount is optional
/// (doc 10 §2's VAT-readiness gap) - typing the VAT shown on the supplier's slip costs 5 seconds
/// and makes history reclaimable/reportable if VAT registration comes later.</summary>
public record CreatePurchaseLineRequest(int ProductId, int? GradeId, decimal Qty, decimal UnitCost, int ShelfLifeDays, decimal? VatAmount = null);
